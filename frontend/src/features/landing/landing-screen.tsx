      ...
      <View style={s.container}>
        ...
        <View testID="landing-hero-globe" onLayout={event => { sections.current.top = event.nativeEvent.layout.y; }} style={[s.hero, mobile && s.heroMobile]}>
          ...
          <View style={[s.heroCopy, mobile && { maxWidth: '100%' }]}>
            ...
            <View style={s.heroKeywords}><Txt style={s.heroKeyword}>IDIOMAS</Txt><View style={s.keywordRule} /><Txt style={s.heroKeyword}>PESSOAS</Txt><View style={s.keywordRule} /><Txt style={s.heroKeyword}>NOVAS POSSIBILIDADES</Txt></View>
          </View>
        </View>
        <View testID="landing-services" onLayout={event => { sections.current.services = event.nativeEvent.layout.y; }} style={s.section}>
          ...
          <View style={[s.services, !wide && { flexWrap: 'wrap' }, mobile && { flexDirection: 'column' }]}>{services.map(service => <Pressable key={service.title} accessibilityRole="button" accessibilityLabel={`Solicitar ${service.title}`} onPress={() => { change('service', service.title); setStep(0); go('quote'); }} style={({ hovered }) => [s.service, hovered && { backgroundColor: colors.elevated, borderColor: '#4B2C35' }, !wide && { minWidth: mobile ? 0 : 280 }, mobile && { padding: 24, flex: 0, flexShrink: 0, flexBasis: 'auto' }]}><View style={s.serviceTop}><View style={s.serviceIcon}><service.icon size={28} color={colors.accent} strokeWidth={1.6} /></View></View><Txt style={s.serviceTitle}>{service.title}</Txt><Txt style={s.serviceText}>{service.text}</Txt><View style={s.serviceBottom}><View /><View style={s.serviceArrow}><ArrowRight size={20} color={colors.accent} /></View></View></Pressable>)}</View>
        </View>
...
const s = StyleSheet.create({
  ...
  services: { flexDirection: 'row', gap: 14 }, service: { flex: 1, minHeight: 250, padding: 26, borderRadius: 18, backgroundColor: '#101011', borderWidth: 1, borderColor: colors.line }, serviceTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }, serviceIcon: { width: 56, height: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 16, backgroundColor: '#211218', borderWidth: 1, borderColor: '#3F202A' }, serviceTitle: { fontSize: 21, lineHeight: 27, fontWeight: '600', marginBottom: 12, letterSpacing: -0.4 }, serviceText: { fontSize: 15, lineHeight: 23, color: colors.muted }, serviceBottom: { flex: 1, minHeight: 46, marginTop: 22, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }, serviceArrow: { width: 42, height: 42, borderRadius: 21, borderWidth: 1, borderColor: '#5D5559', alignItems: 'center', justifyContent: 'center' },
