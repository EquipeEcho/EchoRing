...
export function RequestsScreen() {
  ...
  return <ScrollView contentContainerStyle={[s.page, width < 700 && { padding: 20 }]} keyboardShouldPersistTaps="handled">
    ...
    {visible.map(request => <Pressable key={request.id} accessibilityRole="button" accessibilityLabel={`Abrir solicitação ${request.id}: ${request.title}`} disabled={busy} onPress={() => void open(request)} style={({ hovered }) => [s.requestRow, hovered && { backgroundColor: colors.elevated, borderColor: '#3B2A30' }, width < 700 && { flexDirection: 'column', alignItems: 'stretch' }]}>
...
const s = StyleSheet.create({
  page: { padding: 40, flexGrow: 1 }, summary: { flexDirection: 'row', alignItems: 'center', gap: 16, backgroundColor: colors.surface, borderRadius: 20, borderWidth: 1, borderColor: colors.line, padding: 22, marginBottom: 26 }, summaryIcon: { height: 48, width: 48, borderRadius: 16, backgroundColor: colors.accentSoft, alignItems: 'center', justifyContent: 'center' },
  search: { flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderColor: colors.line, borderRadius: 15, paddingHorizontal: 18, backgroundColor: colors.surface, maxWidth: 500 }, searchInput: { flex: 1, minWidth: 0, minHeight: 50, fontFamily: font, fontSize: 15, color: colors.ink, outlineWidth: 0 }, filters: { flexDirection: 'row', gap: 8, paddingVertical: 20 }, filter: { borderRadius: 14, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surface, paddingVertical: 10, paddingHorizontal: 16 },
  requestRow: { flexDirection: 'row', alignItems: 'center', gap: 20, padding: 20, marginBottom: 12, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 20 }, requestTitle: { fontSize: 19, lineHeight: 26, fontWeight: '500' }, detailTitle: { fontSize: 26, lineHeight: 33, fontWeight: '600' }, customer: { gap: 6, paddingVertical: 16, borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.line }, detailRow: { flexDirection: 'row', justifyContent: 'space-between', gap: 20 }, message: { backgroundColor: colors.canvas, padding: 18, borderRadius: 16, gap: 10 }, label: { fontSize: 14, fontWeight: '600' }, quoteHeading: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingTop: 22, borderTopWidth: 1, borderTopColor: colors.line }, input: { minHeight: 52, borderRadius: 15, backgroundColor: colors.canvas, borderWidth: 1, borderColor: colors.line, paddingHorizontal: 16, fontFamily: font, fontSize: 16, color: colors.ink },
